...
class DistInterpolation(InterpolationModel):
    ...
    def get_cdf(self, start, end, x, all_params):
        ...
        arg = params[:-2]
        if x >= end:
            return distribution.cdf(end, loc=params[-2], scale=params[-1], *arg)
        if x <= start:
            return distribution.cdf(start, loc=params[-2], scale=params[-1], *arg)
...
class PolyInterpolation(InterpolationModel):
    ...
    def get_cdf(self, start, end, x, all_params):
        ...
        if x <= start:
            return 0
        return integrate.quad(np.poly1d(params), start, x,
                              epsrel=1.49e-05)[0]
